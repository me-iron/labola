  ...
  const analytics = React.useMemo((): AnalyticsData => {
    ...
    filteredEvents.forEach(event => {
      ...
      if (typeof event.price === 'number') {
        ...
        priceByRegion.set(regionKey, {
          total: currentRegionPrice.total + event.price,
          count: currentRegionPrice.count + 1
        });

        // By Stadium (Organizer)
        // stadiumKey is already defined above
        const currentStadiumPrice = priceByStadium.get(stadiumKey) || { total: 0, count: 0 };
        priceByStadium.set(stadiumKey, {
          total: currentStadiumPrice.total + event.price,
          count: currentStadiumPrice.count + 1
        });
      }
    });
    ...
    const avgPriceByRegion = Array.from(priceByRegion.entries())
      .map(([region, data]) => ({
    ...
    const avgPriceByStadium = Array.from(priceByStadium.entries())
