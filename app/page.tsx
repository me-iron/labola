...
interface AnalyticsData {
  ...
  avgPriceByRegion: { region: string; avgPrice: number }[];
  avgPriceByStadium: { stadium: string; avgPrice: number }[];
}
...
export default function Dashboard() {
  ...
  const analytics = React.useMemo((): AnalyticsData => {
    ...
    if (typeof e.price === 'number') {
      // By Region
      const regionKey = e.region || 'Unknown';
      if (!acc.priceByRegion[regionKey]) acc.priceByRegion[regionKey] = { total: 0, count: 0 };
      acc.priceByRegion[regionKey].total += e.price;
      acc.priceByRegion[regionKey].count += 1;

      // By Stadium (Organizer)
      const stadiumKey = e.stadium || 'Unknown';
      if (!acc.priceByStadium[stadiumKey]) acc.priceByStadium[stadiumKey] = { total: 0, count: 0 };
      acc.priceByStadium[stadiumKey].total += e.price;
      acc.priceByStadium[stadiumKey].count += 1;
    }

    return acc;
  },
    {
      byDate: {} as Record<string, { supply: number; booked: number; count: number; proceeding: number }>,
      byStadium: {} as Record<string, { supply: number; count: number }>,
      priceByRegion: {} as Record<string, { total: number; count: number }>,
      priceByStadium: {} as Record<string, { total: number; count: number }>,
      totalEvents: 0,
      totalCapacity: 0,
      totalBooked: 0,
      proceedingCount: 0,
  ...
  const byDate = Object.entries(processed.byDate)
    .map(([key, data]) => {
      let dateLabel = key;
      if (period > 1) {
  ...
  const byStadium = Object.entries(processed.byStadium)
    .map(([stadium, data]) => ({ stadium, ...data }))
    .sort((a, b) => b.count - a.count)
    .slice(0, 10);

  const avgPriceByRegion = Object.entries(processed.priceByRegion)
    .map(([region, data]) => ({
      region: lang === 'ko' && DICT.ko.regionMap[region] ? DICT.ko.regionMap[region] : region,
      avgPrice: Math.round(data.total / data.count)
    }))
    .sort((a, b) => b.avgPrice - a.avgPrice);

  const avgPriceByStadium = Object.entries(processed.priceByStadium)
    .map(([stadium, data]) => ({ stadium, avgPrice: Math.round(data.total / data.count) }))
    .sort((a, b) => b.avgPrice - a.avgPrice)
    .slice(0, 10);
  ...
  setAnalytics({
    ...
    totalCapacity: processed.totalCapacity,
    totalBooked: processed.totalBooked,
    proceedingCount: processed.proceedingCount,
    totalOrganizers: processed.organizers.size,
    avgPriceByRegion,
    avgPriceByStadium
  });
  ...
  <div className="min-h-screen bg-neutral-950 text-neutral-100 p-8 font-sans selection:bg-indigo-500/30">
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        <ChartCard title={lang === 'ko' ? DICT.ko.supplyByDate : DICT.ja.supplyByDate}>
          <SupplyChart data={analytics.byDate} lang={lang} />
        </ChartCard>
        <ChartCard title={lang === 'ko' ? DICT.ko.supplyByStadium : DICT.ja.supplyByStadium}>
          <OrganizerChart data={analytics.byStadium} />
        </ChartCard>
      </div>
